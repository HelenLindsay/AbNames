...
The `hgnc` data set is a consistently formatted version of the protein-coding
and gene groups tables from the Human Gene Names Consortium. This includes previous (obsolete) gene names and aliases, which in our experience have been useful for matching to antibody names.  The `hgnc` data set is in long format (one alias per row).
...
We can try to annotate antibodies that could not be annotated using the
...
AbNames has two parts - a collection of gene names and aliases, and information about CITE-seq antibodies.  You could use the first part for mapping CYTOF antibodies to gene identifiers.   
