...
hgnc_results <- hgnc_results %>%
    ...
    dplyr::summarise(dplyr::across(c(HGNC_ID, ENSEMBL_ID, UNIPROT_IDS),
...
ID column, or use dplyr::update_rows.  We will demonstrate the latter approach below. 

```{r}
# Before filling, check how many antibodies were matched.
n_matched <- function(df){
    df %>%
        dplyr::group_by(ID) %>%
        dplyr::summarise(match = any(! is.na(HGNC_ID))) %>%
        dplyr::ungroup() %>%
        dplyr::count(match)
}

original_nmatched <- n_matched(citeseq)
...
citeseq <- citeseq %>%
     AbNames::fillByGroup("Cat_Number", multiple = "mode",
                          fill = c("HGNC_ID", "TotalSeq_Cat", "Vendor",
                                           "ENSEMBL_ID", "UNIPROT_IDS"))
nmatched_after_fill <- n_matched(citeseq)
...
We can also fill some of the missing information using the totalseq_cocktails table.
