...
* Extracting potential gene names from antibody names, 
...
non-ambiguous proteins from the Cell-Surface Protein Atlas (CSPA, Cell-Surface Protein Atlas) and Cellmarker protein database (http://xteam.xbio.top/CellMarker/) have been added.  Mappings between HGNC, Ensembl and NCBI (Entrez) IDs are mostly
...
and only human antibodies and isotype controls are included.  Missing and incomplete Ensembl IDs have been manually corrected.   
...
HGNC by using the totalseq table.  This could also be done as a first matching step as above.  Here, we cannot add any more annotations. 
...
# Problem - adding protein IDs has added ambiguous antigens, e.g. CD98
# match to lower on both tables?
...
# Tasks - getting a correct identifier
# Matching groups (don't necessarily need to correct the identifier mapping in this case)
# Can fill IDs by majority clone / Cat_Number
# KLG / MAFA
# Keep source in ID result table?
# Is TCRg clone B1 = TCRg/d?
# across query columns - na_if equal to antigen and not is antigen
# Replace greek symbols in totalseq
# Check results in matchToCITEseq
# keep ambiguous names in a separate column?
